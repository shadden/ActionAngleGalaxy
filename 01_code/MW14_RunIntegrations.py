...
Rc = 1.
NL = 1.
ham = MWPotential2014_get_hamiltonian([Rc,0.,0.,0.],NL)
...
for J,theta in enumerate(angles):
    print(J)
    ics = np.array([Rc,0.,v_init * np.cos(theta), v_init * np.sin(theta)])
...
np.savez_compressed(savedir + "mw14_integations_{}".format(I),orbit=all_orbits,times=times,xR=all_xR,xz=all_xz)
