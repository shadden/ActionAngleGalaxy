...
def uv_lims_to_ELI3(u1,u2,nu,e):
...
def uv_lims_to_actions_freqs(u1,u2,nu,e):
    E,L,I3 = uv_lims_to_ELI3(u1,u2,nu,e)
    G = lambda x: Gfn(x,e)
    Jr_ig = lambda u: np.sqrt(E*np.sinh(u)**2 - I3 - 0.5 * L*L / np.sinh(u)**2 / e**2 - G(np.cosh(u)))
    Jz_ig = lambda v: np.sqrt(E*np.sin(v)**2  + I3 - 0.5 * L *L / np.sin(v)**2 / e**2 + G(np.cos(v)))
    
    dJr_dE_ig = lambda u: 0.5 * np.sinh(u)**2 / np.sqrt(E*np.sinh(u)**2 - I3 - 0.5 * L*L / np.sinh(u)**2 / e**2 - G(np.cosh(u)))
    dJz_dE_ig = lambda v: 0.5 * np.sin(v)**2  / np.sqrt(E*np.sin(v)**2  + I3 - 0.5 * L *L / np.sin(v)**2 / e**2 + G(np.cos(v)))

    dJr_dI3_ig = lambda u: -0.5 / np.sqrt(E*np.sinh(u)**2 - I3 - 0.5 * L*L / np.sinh(u)**2 / e**2 - G(np.cosh(u)))
    dJz_dI3_ig = lambda v:  0.5 / np.sqrt(E*np.sin(v)**2  + I3 - 0.5 * L *L / np.sin(v)**2 / e**2 + G(np.cos(v)))

    dJr_dL_ig = lambda u: (- 0.5 * L / np.sinh(u)**2 / e**2) / np.sqrt(E*np.sinh(u)**2 - I3 - 0.5 * L*L / np.sinh(u)**2 / e**2 - G(np.cosh(u)))
    dJz_dL_ig = lambda v: (- 0.5 * L / np.sin(v)**2 / e**2)  / np.sqrt(E*np.sin(v)**2  + I3 - 0.5 * L*L / np.sin(v)**2 / e**2 + G(np.cos(v)))
...
def ELI3_to_pu_sq_pv_sq(E,L,I3,u0,v0,e):
    sh_sq = np.sinh(u0)**2
    s_sq = np.sin(v0)**2
    U = Gfn(np.cosh(u0),e)
    V = Gfn(np.cos(v0),e)
    Lsq = L*L
    esq= e*e
    pu_sq = 2 * esq * (E * sh_sq - I3 - 0.5* Lsq / esq / sh_sq - U)
    pv_sq = 2 * esq * (E *  s_sq + I3 - 0.5* Lsq / esq / s_sq  + V)
    return pu_sq, pv_sq
